...
def get_pump_status():
    """Get pump status by sending command to ESP32 and waiting for response"""
    ...
    # Generate unique command ID for pump status query
    command_id = f"cmd_pump_status_{int(time.time() * 1000) % 1000000}"

    # Create pump status command
    pump_status_command = {
        "messageType": "command",
        "commandType": "query",
        "version": "1.0",
        ...
        "deviceId": device_id,
        "command": {
            "action": "get_pump_status"
        }
    }

    log_json('/api/device/sensor/pump_status', f'SENDING COMMAND (deviceId={device_id})', pump_status_command)
    ...
    with lock:
        ...
        command_queues[device_id].append(pump_status_command)
        ...
        command_history[command_id]['command'] = pump_status_command
        command_history[command_id]['status'] = 'queued'

    # Wait for ESP32 response
    timeout_seconds = 10.0  # Shorter timeout for sensor queries
    ...
    while time.time() - start_time < timeout_seconds:
        with lock:
            if command_id in command_history and 'result' in command_history[command_id]:
                ...
                log_json('/api/device/sensor/pump_status', f'RESPONSE (ESP Result)', esp_result)
                return jsonify(esp_result), 200

        time.sleep(poll_interval)

    # Timeout - ESP32 didn't respond in time
    timeout_data = {
        "messageType": "command_response",
        "version": "1.0",
        "deviceId": device_id,
        "response": {
            "status": "error",
            "statusCode": 504,
            "message": "ESP32 did not respond to pump status query",
            "data": {
                "component": "pump_01",
                "pumpState": "unknown",
                "operation": "unknown",
    ...
    log_json('/api/device/sensor/pump_status', 'RESPONSE (Timeout)', timeout_data)
